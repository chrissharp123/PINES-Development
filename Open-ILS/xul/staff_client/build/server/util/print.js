        ...
        try {
            ...
            switch(content_type) {
                case 'text/html' :
                    ...
                    w = obj.win.open('data:text/html,<html id="top"><head><script src="/xul/rel_1_6_1_4/server/main/JSAN.js"></script><script src="' + window.escape(jsrc) + '"></script></head><body onload="try{my_init();}catch(E){alert(E);}">' + window.escape(msg) + '</body></html>','receipt_temp','chrome,resizable');
                    w.minimize();
                    w.go_print = function() { 
                        try {
                            obj.NSPrint(w, silent, params);
